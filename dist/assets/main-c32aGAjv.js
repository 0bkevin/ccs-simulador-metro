import{s as de,G as pe,R as ue,C as me,V as he,O as ge,l as fe,W as be,T as ve}from"./style-Dm_zbBsP.js";import{R as we,c as ye,a as Ae,p as ne,b as Ce,d as re,g as Te,e as xe,f as ie,s as ke,h as Ee}from"./station-camera-CoAlCM5c.js";const Se=`# Línea 1 del Metro de Caracas: investigación para el juego
...
`,se={color:"#d71920",realStationCount:22},Y=se.color,Me=[{id:"cano-amarillo",name:"Caño Amarillo",distance:0,color:Y,type:"elevated",platformLayout:"side",railCenters:[0,4],interchange:null,detail:"Estación elevada junto al antiguo eje ferroviario Caracas–La Guaira, con cubierta reticulada amarilla y andenes laterales."},{id:"capitolio",name:"Capitolio",distance:900,color:Y,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Conexión peatonal con El Silencio (Línea 2)",detail:"Estación profunda del centro con mezzanina y conexión peatonal hacia El Silencio."},{id:"bellas-artes",name:"Bellas Artes",distance:1800,color:Y,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación profunda con andén central bajo el eje cultural de Bellas Artes."},{id:"plaza-venezuela",name:"Plaza Venezuela",distance:2800,color:Y,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Líneas 3 y 4 (conexiones del complejo)",detail:"Complejo de transferencia con niveles y pasillos de conexión de alta demanda."},{id:"altamira",name:"Altamira",distance:3900,color:Y,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación de tres niveles bajo la plaza y avenida Francisco de Miranda, con andén central y mezzanina."}],oe=Me.map((v,y)=>({...v,...de.stations[y]}));function Ie(v,y,n,g){var H,q;const e=((H=g.train.interior)==null?void 0:H.cars)||[],o=new pe;o.name="Blender passenger lighting",v.add(o),we.init();const I=(((q=g.lighting)==null?void 0:q.trainAreaLights)||[]).map(b=>{const M=new ue(new me().fromArray(b.color),b.power/(b.width*b.height),b.width,b.height);return M.name=b.name,M.position.fromArray(b.position),M.quaternion.fromArray(b.quaternion),M.userData.carIndex=Number(b.collection.slice(-2)),M.visible=!1,o.add(M),M});let C=1,T="saloon",t=!1,k=0,l=null,f=0,A=0,x=6.63;const m=new he(.22,2.57,0);function S(b=C,M="saloon"){C=Math.max(1,Math.min(e.length||7,Number(b)||1)),T=["saloon","seats","doors","gangway"].includes(M)?M:"saloon";const R=e[C-1];f=0,A=0,x=6.63,m.set(.22,(R==null?void 0:R.eyeY)||2.57,0),T==="seats"&&(x=4.25,m.x=-.22,m.y=2.22,f=-1.3,A=-.36),T==="doors"&&(x=2.333,m.x=.28,f=-Math.PI/2,A=-.08),T==="gangway"&&(x=-8.6,m.x=.22),k++}function L(b,M,R=b){t=M&&e.length>0;const d=e[C-1];t&&d&&(y.position.set(m.x,m.y,b+d.center+d.direction*x),y.lookAt(y.position.x+d.direction*Math.sin(f)*Math.cos(A),y.position.y+Math.sin(A),y.position.z-d.direction*Math.cos(f)*Math.cos(A)));const p=t?C:e.reduce((r,w)=>{var s;return Math.abs(b+w.center-R)<Math.abs(b+(((s=e[r-1])==null?void 0:s.center)||0)-R)?w.index:r},1);for(const r of I)r.visible=v.visible&&Math.abs(r.userData.carIndex-p)<=(t?1:0)}function E(b){x=Math.max(-9.6,Math.min(7.6,Number(b)||0)),k++}const c=n==null?void 0:n.domElement,_=b=>{!t||b.button!==0||(l={id:b.pointerId,x:b.clientX,y:b.clientY},c.setPointerCapture(b.pointerId))},B=b=>{!t||!l||l.id!==b.pointerId||(f-=(b.clientX-l.x)*.004,A=Math.max(-1.1,Math.min(1.1,A-(b.clientY-l.y)*.004)),l.x=b.clientX,l.y=b.clientY,k++)},z=()=>{l=null};return c==null||c.addEventListener("pointerdown",_),c==null||c.addEventListener("pointermove",B),c==null||c.addEventListener("pointerup",z),c==null||c.addEventListener("pointercancel",z),{select:S,update:L,move:E,cars:e,lights:I,get revision(){return k},get state(){return{carIndex:C,view:T,travel:x,enabled:t}},dispose(){c==null||c.removeEventListener("pointerdown",_),c==null||c.removeEventListener("pointermove",B),c==null||c.removeEventListener("pointerup",z),c==null||c.removeEventListener("pointercancel",z)}}}function je(v,y,n=[],g){if(!(g!=null&&g.train)||!(g!=null&&g.environment))throw new Error("Los modelos Blender aún no están disponibles.");const e=new v.Scene,o=ye(e,y,g.manifest),I=Ae(e);e.fog=new v.Fog(1581354,260,900);const C=new v.Group;C.name="Caracas Metro L1 · Blender export",e.add(C);const T=g.environment;T.name="Blender environment · five stops",C.add(T),ne(T,y);const t=[];T.traverse(d=>{!d.isMesh&&d.userData.sourceCollection&&t.push(d)});const k=n.map(d=>t.find(p=>p.userData.sourceCollection===d.collection));if(k.some(d=>!d))throw new Error("Falta una estación Blender en el entorno del juego.");const l=g.train;l.name="Blender train · 7 cars",C.add(l),ne(l,y);const f={forward:new v.PerspectiveCamera(70,1,.05,1200),exterior:new v.PerspectiveCamera(68,1,.1,900),platform:new v.PerspectiveCamera(68,1,.1,900),inspection:new v.PerspectiveCamera(68,1,.2,700),interior:new v.PerspectiveCamera(72,1,.035,700)},A=Ie(l,f.interior,y,g.manifest);let x="platform",m=null,S=0,L=0;const E=y?new ge(f.inspection,y.domElement):null,c=Ce({camera:f.inspection,controls:E,environment:T});E&&(E.enabled=!1,E.enableDamping=!1,E.addEventListener("change",()=>{c.adjusting||(c.constrain(),S++)}));const _=()=>m?f.inspection:f[x];function B(d){return x=["forward","exterior","platform","interior"].includes(d)?d:"platform",S++,_()}function z(d=L,p="platform"){d=Math.max(0,Math.min(n.length-1,Number(d)||0));const r=Te(d,p,f.inspection.aspect);m={index:d,...r};for(const s of t)s.visible=r.collections.includes(s.userData.sourceCollection);re(y,r),I.show(d,r.kind);const w=n[d].distance;return l.visible=["platform","detail","concourse"].includes(r.kind)&&l.position.z>=w-148&&l.position.z<=w+150,E&&(E.enabled=!0),c.setView(d,r),o.focus(f.inspection.position,r.collection,r.exterior,["plan","section"].includes(r.kind)),S++,m}function H(){m=null,re(y,{}),I.hide(),E&&(E.enabled=!1),c.clear(),t.forEach(d=>{d.visible=!0}),l.visible=!0,S++}function q(d){(m==null?void 0:m.kind)!=="tunnel"||!Number.isFinite(Number(d))||(c.moveTunnel(d),o.focus(f.inspection.position,m.collection),S++)}function b(d=.016,p=0){l.position.set(0,0,Number(p)||0);const r=l.position.z;f.forward.position.set(0,2.5,r+5),f.forward.lookAt(0,2,r+15);const w=A.cars[A.state.carIndex-1],s=x==="interior"&&w?r+w.center+w.direction*A.state.travel:x==="platform"?r-84:r+4.6;L=n.findIndex(P=>s<=P.distance+5),L<0&&(L=n.length-1);const h=n[L],N=s>=h.distance-145&&s<=h.distance+5,D=["Bellas Artes","Altamira"].includes(h==null?void 0:h.name),O=D?2.5:(h==null?void 0:h.name)==="Capitolio"?-2.65:-3.9;f.platform.position.set(N?O:0,2.73,N?r-84:r+5),f.platform.lookAt(N?O:0,2.98,N?r-69:r+23);const U=n.some(P=>r+4.6>=P.distance-145&&r+4.6<=P.distance+5);if(f.exterior.position.set(U?D?7.5:O:0,2.73,r+4.6),f.exterior.lookAt(0,2.1,U?r-4:r+22),A.update(r,x==="interior"&&!m,_().position.z),m)c.constrain(),o.focus(f.inspection.position,m.collection,m.exterior,["plan","section"].includes(m.kind));else{const P=_().position.clone(),V=n.find(j=>P.z>=j.distance-145&&P.z<=j.distance+5),K=Math.max(0,n.findLastIndex(j=>P.z>j.distance+5));o.focus(P,(V==null?void 0:V.collection)||`Tunnel ${n[K].id}`)}}function M(d,p){Object.values(f).forEach(r=>{r.aspect=d/Math.max(1,p),r.updateProjectionMatrix()})}function R(){C.traverse(d=>{var p;d.geometry&&d.geometry.dispose(),(p=d.material)!=null&&p.dispose&&d.material.dispose()}),E==null||E.dispose(),c.dispose(),o.dispose(),A.dispose(),I.dispose()}return{scene:e,root:C,train:l,cameras:f,get camera(){return _()},get inspection(){return m},get activeStation(){return L},get renderRevision(){return S+A.revision},get doorFraction(){return 0},setCameraMode:B,inspectStation:z,leaveInspection:H,moveTunnel:q,update:b,resize:M,dispose:R,controls:E,stationAssemblies:k,railPaths:[],lighting:o,interior:A,cameraGuard:c,assetSource:g.source,manifest:g.manifest}}function Ne(v=[],{routeEnd:y=1/0}={}){var C,T;const n=((C=v[0])==null?void 0:C.distance)??0,g=Math.max(((T=v.at(-1))==null?void 0:T.distance)??n,y);let e;const o=()=>({position:n,speed:0,target:0,doorsOpen:!0,dwell:3,started:!1,missed:!1,complete:!1,paused:!1,score:100,serviceCount:0});return e=o(),{get state(){return{...e}},start(){return e.started=!0,this},reset(){return e=o(),e.started=!0,this},pause(t=!e.paused){return e.paused=t,this},toggleDoors(){const t=v[e.target],k=t&&Math.abs(e.position-t.distance)<=12;return e.speed>.04||!k||e.missed||e.complete?this.state:(e.doorsOpen?e.dwell<=0&&(e.doorsOpen=!1,e.serviceCount++,e.target===v.length-1?e.complete=!0:e.target++):(e.doorsOpen=!0,e.dwell=3),this.state)},recover(){const t=v[e.target];return!t||e.complete||!e.missed?this.state:(e.position=t.distance,e.speed=0,e.doorsOpen=!0,e.dwell=3,e.missed=!1,e.score=Math.max(0,e.score-15),this.state)},tick(t,k={}){if(!e.started||e.paused||e.complete)return this.state;const l=Math.min(Math.max(Number(t)||0,0),.05);e.doorsOpen&&(e.dwell=Math.max(0,e.dwell-l));const f=!!k.throttle&&!e.doorsOpen&&!e.missed;k.emergency?e.speed=Math.max(0,e.speed-3*l):k.brake?e.speed=Math.max(0,e.speed-2.5*l):f?e.speed=Math.min(18,e.speed+1.15*l):e.speed=Math.max(0,e.speed-.22*l),e.position+=e.speed*l,e.position>=g&&(e.position=g,e.speed=0);const A=v[e.target];return A&&e.position>A.distance+12&&!e.missed&&(e.missed=!0,e.score=Math.max(0,e.score-20)),this.state}}}const Pe=[{title:"Metro De Caracas Compilación Trenes Alstom y CAF",url:"https://www.youtube.com/watch?v=Ej0X90zrtNg",creator:"Dani215 Railway",date:"2024-07-17",license:"No redistributable licence verified on the source page; reference-only",authenticity:"creator's description identifies Line 1 CAF Renfe Serie 6 footage (61047–61080), alongside Alstom footage; no audio copied"},{title:"OpenBVE Venezuela CAF Serie 6 asset pack (external download)",url:"https://www.mediafire.com/file/om2u9qlgt7gaahw/Pack_Series_10000_y_60000_MCCS_FINAL.zip/file",creator:"OpenBVE Venezuela / uploader credited on the pack page",license:"No reuse licence or permission statement found in the page/archive; do not bundle",authenticity:"CAF S6 train asset includes WAV filenames, but the archive does not establish that they are field recordings"},{title:"Caracas Metro compilation (Line 1, 2006–2013)",url:"https://www.youtube.com/watch?v=86YCTUO5gQw",creator:"MonteBRujaFM",license:"No redistributable licence verified on the source page; reference-only",authenticity:"creator describes Caracas Line 1 rolling-stock footage; audio remains copyrighted"},{title:"Caracas Metro 10000 and 20000 series sound examples",url:"https://www.youtube.com/watch?v=gtgx51bclew",creator:"YouTube uploader (linked by r/transit discussion)",license:"No redistributable licence verified on the source page; reference-only",authenticity:"identified as older Caracas Alstom 10000/20000-series stock, not CAF 60000-series; not bundled"},{title:"Caracas Metro 10000 and 20000 series sound example 2",url:"https://www.youtube.com/watch?v=cbB9gqDMQ0o",creator:"YouTube uploader (linked by r/transit discussion)",license:"No redistributable licence verified on the source page; reference-only",authenticity:"identified as older Caracas Alstom 10000/20000-series stock, not CAF 60000-series; not bundled"}],Oe={authenticity:"procedural gameplay fallback; no bundled Caracas recording"},Q=(v,y=0,n=1)=>Math.max(y,Math.min(n,v));function _e(v,y=2){const n=v.createBuffer(1,v.sampleRate*y,v.sampleRate),g=n.getChannelData(0);let e=0;for(let o=0;o<g.length;o+=1)e=e*.965+(Math.random()*2-1)*.18,g[o]=e;return n}function Be({now:v=()=>performance.now()/1e3,contextFactory:y=null}={}){let n=null,g=!1,e=.65,o=!1,I=null,C=v(),T=0,t=0,k=[],l=null,f=null,A=[],x=[],m=null,S=null,L=null,E=null,c=null;function _(p,r,w,s=0){const h=n.createOscillator(),N=n.createGain();return h.type=p,h.frequency.value=r,N.gain.value=s,h.connect(N).connect(w),h.start(),k.push(h,N),{osc:h,gain:N}}function B(){if(n||o)return;const p=y||globalThis.AudioContext||globalThis.webkitAudioContext;if(!p)return;n=y?y():new p,l=n.createGain(),l.gain.value=0,f=n.createDynamicsCompressor(),f.threshold.value=-18,f.knee.value=16,f.ratio.value=7,f.attack.value=.008,f.release.value=.18,l.connect(f).connect(n.destination),A=[56,112,168].map((U,P)=>{const V=_(P===1?"triangle":"sine",U,l,0);return x.push(V.gain),V.osc});const r=n.createBufferSource();r.buffer=_e(n),r.loop=!0;const w=n.createBiquadFilter();w.type="lowpass",w.frequency.value=760,w.Q.value=.45,m=n.createGain(),m.gain.value=0,r.connect(w).connect(m).connect(l),r.start(),k.push(r,w,m),S=n.createGain(),S.gain.value=0;const s=_("sawtooth",145,S,.07),h=n.createBiquadFilter();h.type="bandpass",h.frequency.value=820,h.Q.value=2.1,S.disconnect(),S.connect(h).connect(l),k.push(h,s),L=n.createGain(),L.gain.value=.002,L.connect(l);const N=_("sine",47,L,.6);k.push(N),E=n.createGain(),E.gain.value=0;const D=n.createOscillator(),O=n.createBiquadFilter();D.type="sawtooth",D.frequency.value=1700,O.type="bandpass",O.frequency.value=1350,O.Q.value=2,D.connect(O).connect(E).connect(l),D.start(),k.push(D,O,E),c=n.createGain(),c.gain.value=1,c.connect(l),k.push(c)}const z=(p,r,w=.06)=>{if(!n||!p)return;const s=n.currentTime;p.cancelScheduledValues(s),p.setTargetAtTime(r,s,w)};function H(){if(!n||!g||!E)return;const p=n.currentTime;[660,880].forEach((r,w)=>{const s=n.createOscillator(),h=n.createGain();s.type="sine",s.frequency.value=r,h.gain.setValueAtTime(1e-4,p+w*.16),h.gain.exponentialRampToValueAtTime(.06,p+w*.16+.012),h.gain.exponentialRampToValueAtTime(1e-4,p+w*.16+.22),s.connect(h).connect(c),s.start(p+w*.16),s.stop(p+w*.16+.25)})}function q(p,r={}){const w=Q(Number(p)||0,0,.1);C=v();const s=!!r.doorsOpen;if(I!==s&&I!==null&&(I===!0&&s===!1&&H(),t=1),I=s,!n||!g||o)return;const h=Q((Number(r.speed)||0)/18),N=!!(r.brake||r.emergency);t=Math.max(0,t-w/.8);const O=r.camera==="exterior"||r.cameraMode==="exterior"?.68:1,U=!!r.started&&!r.paused&&!r.complete&&!s,P=!!r.started&&!r.paused&&!r.complete,V=U?O:0;z(l.gain,e*(P?O*(s?.055:.1375+h*.275):0),.08),A.forEach((j,F)=>z(j.frequency,[56,113,171][F]+h*[130,260,390][F],.1));const K=r.throttle?1:.35;if(x.forEach((j,F)=>z(j.gain,V*(.018-F*.003)*(.15+h)*K,.1)),z(m.gain,V*(.018+h*.035),.14),z(S.gain,V*(N?.8:0)*h,.09),z(L.gain,P?O*.012:0,.2),z(E.gain,t*.012,.08),T+=w*(1.5+h*13),U&&h>.03&&T>=1){T%=1;const j=n.createOscillator(),F=n.createGain(),$=n.currentTime;j.type="triangle",j.frequency.value=540+Math.random()*90,F.gain.setValueAtTime(1e-4,$),F.gain.exponentialRampToValueAtTime(Math.max(2e-4,.018*h),$+.004),F.gain.exponentialRampToValueAtTime(1e-4,$+.045),j.connect(F).connect(l),j.start($),j.stop($+.05)}}async function b(p){return o?!1:(g=!!p,g&&(B(),n&&n.state==="suspended"&&n.resume&&typeof n.startRendering!="function"&&await n.resume()),n&&z(l.gain,g?e*.001:0,.04),g)}function M(){I=null,T=0,t=0,n&&z(l.gain,0,.04)}function R(p){return e=Q(Number(p)/100,0,1),n&&g&&z(l.gain,e*.001,.04),e*100}function d(){var p;o=!0,g=!1,k.forEach(r=>{var w,s;try{(w=r.stop)==null||w.call(r),(s=r.disconnect)==null||s.call(r)}catch{}}),(p=n==null?void 0:n.close)==null||p.call(n),k=[],n=null}return{setEnabled:b,setVolume:R,update:q,reset:M,dispose:d,status:()=>({enabled:g,available:!!n,running:!!(n&&n.state==="running"),lastUpdate:C})}}async function Fe(){var X;const v=document.querySelector("#app");v.innerHTML='<div class="asset-loading" role="status"><div class="asset-loading-mark">M</div><div class="asset-loading-title">Cargando exportación Blender</div><div class="asset-loading-detail" id="assetLoadingDetail">Validando manifest.json…</div></div>';let y;try{y=await fe({onProgress:(a,u)=>{const i=document.querySelector("#assetLoadingDetail");i&&(i.textContent=`${a} · ${Math.round(u*100)}%`)}})}catch(a){const u=String((a==null?void 0:a.message)||a);throw v.innerHTML=`<div class="asset-error" role="alert"><div class="asset-loading-mark">!</div><h1>No se pudo abrir el mundo Blender</h1><p>${u.replace(/[&<>]/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[i])}</p><p>Comprueba que <code>public/models/blender/manifest.json</code>, <code>train.glb</code> y <code>environment.glb</code> estén publicados y vuelve a cargar.</p><button onclick="location.reload()">REINTENTAR</button></div>`,a}const n=(Array.isArray(oe)?oe:[]).map((a,u)=>{var i;return{...a,...y.manifest.stations[u]||{},distance:Number(((i=y.manifest.stations[u])==null?void 0:i.distance)??a.distance)}});v.replaceChildren();const g=new be({antialias:!0,powerPreference:"low-power"});xe(g),g.setSize(innerWidth,innerHeight),v.append(g.domElement);const e=je(ve,g,n,y);e.resize(innerWidth,innerHeight);const o=Ne(n,{routeEnd:ie(n.length-1).max-5}),I=Be();window.__metro={world:e,sim:o,renderer:g,assetSource:y.source,manifest:y.manifest,assetsReady:!0};const C=document.createElement("div");C.className="ui",C.innerHTML=`
...
<div class="overlay" id="sourceModal" style="display:none"><div class="card source-card"><button class="corner" id="closeSources">CERRAR ×</button><div class="eyebrow">Documentación</div><h1>Fuentes</h1><p>Investigación sobre diseño, estaciones, trenes y mecánica de la Línea 1. Fuentes primarias consultadas:</p><div class="source-links"><a href="https://openjicareport.jica.go.jp/pdf/11789237_03.pdf" target="_blank" rel="noreferrer">JICA · datos de línea</a><a href="https://www.aschinfraestructuras.com/linea-caracas" target="_blank" rel="noreferrer">ASCH · rehabilitación de vía</a><a href="https://admin.cafmobility.com/uploads/281_CAF_Catalogo_General_ES_601604d06c.pdf" target="_blank" rel="noreferrer">CAF · catálogo</a><a href="https://www.alstom.com/fr/press-releases-news/2005/9/ALSTOM-remporte-un-contrat-cle-en-main-pour-le-Metro-de-Caracas-au-Venezuela-20050916" target="_blank" rel="noreferrer">Alstom · Metro de Caracas</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery1.htm" target="_blank" rel="noreferrer">UrbanRail · Altamira y Bellas Artes</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery2.htm" target="_blank" rel="noreferrer">UrbanRail · Capitolio</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery4.htm" target="_blank" rel="noreferrer">UrbanRail · Plaza Venezuela</a><a href="https://fundaayc.com/2024/07/14/algo-mas-sobre-la-postal-no-411/" target="_blank" rel="noreferrer">Fundación Arquitectura y Ciudad · Altamira</a><a href="https://recyt.fecyt.es/index.php/CyTET/article/download/83795/61863/276024" target="_blank" rel="noreferrer">Bemergui · arquitectura de estaciones</a></div><p><small>La grabación real es una referencia externa. El audio del juego es procedural; no se redistribuye la grabación.</small></p><button class="secondary" id="realListen">ESCUCHAR REFERENCIA REAL CAF/ALSTOM</button><iframe id="realFrame" title="Referencia real del Metro de Caracas" style="display:none;width:100%;aspect-ratio:16/9;border:0;margin-top:1rem" allow="autoplay; encrypted-media" allowfullscreen></iframe><div id="audioSources"></div><pre id="researchText"></pre><pre id="stationResearchText"></pre></div></div>`,v.append(C);const T=document.createElement("section");T.className="station-review-ui in-game-stations",T.hidden=!0,T.setAttribute("aria-label","Explorar estaciones"),T.innerHTML='<button id="returnToGame" class="back-link">← VOLVER A CONDUCIR</button><div class="review-kicker">EXPLORAR ESTACIÓN · JUEGO EN PAUSA</div><h1 id="gameStationTitle"></h1><p id="gameStationDetail"></p><label>ESTACIÓN <select id="gameStationSelect" aria-label="Estación para explorar"></select></label><div class="review-actions"><button data-station-view="platform">ANDÉN</button><button data-station-view="detail">DETALLE</button><button data-station-view="concourse">MEZZANINA</button><button data-station-view="entrance">ACCESO</button><button data-station-view="south">PLAZA SUR</button><button data-station-view="plan">PLANTA</button><button data-station-view="section">CORTE</button><button data-station-view="tunnel">TÚNEL</button></div><p id="gameStationMetrics" class="station-metrics"></p><label id="gameTunnelTravelLabel" hidden>RECORRER TÚNEL <input id="gameTunnelTravel" type="range" step="1" aria-label="Recorrer túnel en metros"></label><p class="review-hint">Planta y corte: geometría del modelo en metros; medidas de estación estimadas.</p><p class="review-hint">Arrastra para mirar · rueda para acercarte.<br>Tu recorrido se conserva. Esc para volver.</p>',C.append(T);const t=a=>C.querySelector("#"+a),k=t("stationList");n.forEach((a,u)=>{const i=document.createElement("div");i.className="station",i.textContent=a.name,i.dataset.i=u,k.append(i)}),t("researchText").textContent=Se,t("stationResearchText").textContent=ze+`

`+Le;const l=document.createElement("pre");l.textContent=Re,t("stationResearchText").before(l);const f=document.createElement("a");f.href="https://commons.wikimedia.org/wiki/File:Metro_de_caracas_linea_1.jpg",f.target="_blank",f.rel="noopener noreferrer",f.textContent="CAF Serie 6 · fotografía original usada para el modelo",C.querySelector(".source-links").prepend(f),t("audioSources").textContent=Oe.authenticity,Pe.forEach(a=>{const u=document.createElement("a");u.href=a.url,u.target="_blank",u.rel="noopener noreferrer",u.textContent=`${a.title} — ${a.creator}`,u.style.display="block",t("audioSources").append(u)}),t("realListen").onclick=()=>{t("realFrame").src="https://www.youtube-nocookie.com/embed/Ej0X90zrtNg",t("realFrame").style.display="block"};let A=!1,x=!1,m=!1,S=0;const L=["platform","exterior","forward","interior"],E={platform:"ANDÉN",exterior:"EXTERIOR",forward:"MARCHA",interior:"INTERIOR"};t("camera").textContent="VISTA: ANDÉN";let c=!1,_=!1,B=!1,z=!1,H=!1;const q=t("gameStationSelect");n.forEach((a,u)=>{const i=document.createElement("option");i.value=u,i.textContent=a.name,q.append(i)});function b(a=e.activeStation,u="platform"){e.inspection||(H=o.state.paused,h(!1),m=!1,o.pause(!0));const i=e.inspectStation(a,u),J=ke[i.index];q.value=i.index,t("gameStationTitle").textContent=J.name,t("gameStationDetail").textContent=J.detail,t("gameStationMetrics").textContent=Ee(i.index,i.kind),t("gameTunnelTravelLabel").hidden=i.kind!=="tunnel";const G=ie(i.index);t("gameTunnelTravel").min=G.min,t("gameTunnelTravel").max=G.max,t("gameTunnelTravel").value=e.camera.position.z,T.querySelectorAll("[data-station-view]").forEach(W=>{W.hidden=!i.views.includes(W.dataset.stationView),W.classList.toggle("active",W.dataset.stationView===i.kind),W.setAttribute("aria-pressed",W.dataset.stationView===i.kind)}),T.hidden=!1,R.hidden=!0,C.classList.add("station-inspecting")}function M(){e.leaveInspection(),T.hidden=!0,C.classList.remove("station-inspecting"),R.hidden=L[S]!=="interior",o.pause(H)}q.onchange=()=>b(Number(q.value)),T.querySelectorAll("[data-station-view]").forEach(a=>{a.onclick=()=>b(e.inspection.index,a.dataset.stationView)}),t("returnToGame").onclick=M,t("gameTunnelTravel").oninput=a=>{e.moveTunnel(a.target.value),a.target.value=e.camera.position.z};const R=document.createElement("section");R.className="interior-controls",R.hidden=!0,R.setAttribute("aria-label","Interior del tren"),R.innerHTML='<div class="route-label">Salón de pasajeros · CAF</div><label>COCHE <select id="interiorCar" aria-label="Coche del tren"></select></label><div class="interior-presets"><button data-interior-view="saloon">SALÓN</button><button data-interior-view="seats">ASIENTOS</button><button data-interior-view="doors">PUERTAS</button><button data-interior-view="gangway">INTERCONEXIÓN</button></div><label>RECORRER COCHE <input id="interiorTravel" type="range" min="-9.6" max="7.6" step=".01" value="6.63" aria-label="Posición dentro del coche"></label><p>Arrastra para mirar alrededor.<br>W / S siguen controlando el tren.</p>',C.append(R);for(let a=1;a<=7;a++){const u=document.createElement("option");u.value=a,u.textContent=`${a} · ${a===1||a===7?"Extremo":"Intermedio"}`,t("interiorCar").append(u)}function d(a="saloon"){e.interior.select(Number(t("interiorCar").value),a),t("interiorTravel").value=e.interior.state.travel,R.querySelectorAll("[data-interior-view]").forEach(u=>{u.classList.toggle("active",u.dataset.interiorView===a),u.setAttribute("aria-pressed",u.dataset.interiorView===a)})}t("interiorCar").onchange=()=>d(),t("interiorTravel").oninput=a=>e.interior.move(a.target.value),R.querySelectorAll("[data-interior-view]").forEach(a=>{a.onclick=()=>d(a.dataset.interiorView)});function p(a){S=a,e.setCameraMode(L[a]),t("camera").textContent=`VISTA: ${E[L[a]]}`,R.hidden=L[a]!=="interior"}const r=document.createElement("button");r.id="enterInterior",r.className="inspect",r.textContent="INTERIOR DEL TREN",r.onclick=()=>{p(3),d()},t("camera").after(r);const w=document.createElement("button");w.className="secondary",w.textContent="VER INTERIOR",w.onclick=()=>{o.start(),t("intro").style.display="none",p(3),d()},t("inspectIntro").after(w);function s(a){t("toast").textContent=a,t("toast").classList.add("show"),clearTimeout(s.timer),s.timer=setTimeout(()=>t("toast").classList.remove("show"),1700)}function h(a=!1){A=a,x=a}function N(){if(B||e.inspection)return;const a=o.state,u=o.toggleDoors();a.doorsOpen!==u.doorsOpen?s(u.doorsOpen?"Puertas abiertas":"Puertas cerradas"):a.dwell>0?s("Espera el embarque"):s("Fuera de zona de parada")}function D(){B||(h(!1),m=!1,_=o.state.paused,o.pause(!0),I.update(0,{...o.state,paused:!0,doorsOpen:!0}),B=!0,t("sourceModal").style.display="grid")}function O(){B&&(B=!1,t("sourceModal").style.display="none",t("realFrame").src="",t("realFrame").style.display="none",o.pause(_),I.reset())}function U(){e.inspection&&M(),h(!1),m=!1,p(0);const a=o.reset();return t("intro").style.display="none",t("complete").style.display="none",z=!1,s("Servicio reiniciado"),a}function P(a){if(e.inspection){a.code==="Escape"&&(a.preventDefault(),M());return}B||a.repeat||!o.state.started||a.target instanceof HTMLInputElement||a.target instanceof HTMLSelectElement||a.target instanceof HTMLTextAreaElement||!["KeyW","ArrowUp","KeyS","ArrowDown","Space","KeyE","KeyC","KeyP","KeyR"].includes(a.code)||(a.preventDefault(),(a.code==="KeyW"||a.code==="ArrowUp")&&(A=!0),(a.code==="KeyS"||a.code==="ArrowDown")&&(x=!0),a.code==="Space"&&(m=!0,s("Freno de emergencia")),a.code==="KeyE"&&N(),a.code==="KeyC"&&p((S+1)%L.length),a.code==="KeyP"&&(o.pause(),s(o.state.paused?"Pausa":"Continuar")),a.code==="KeyR"&&U())}function V(a){(a.code==="KeyW"||a.code==="ArrowUp")&&(A=!1),(a.code==="KeyS"||a.code==="ArrowDown")&&(x=!1)}addEventListener("keydown",P),addEventListener("keyup",V),addEventListener("blur",()=>{h(!1),m=!1,o.pause(!0)}),document.addEventListener("visibilitychange",()=>{document.hidden&&(h(!1),m=!1,o.pause(!0))}),t("start").onclick=()=>{o.start(),t("intro").style.display="none",s("Caño Amarillo · servicio listo")},t("doors").onclick=N,t("pause").onclick=()=>{o.pause(),s(o.state.paused?"Pausa":"Continuar")},t("recover").onclick=()=>{o.recover(),s("Tren recuperado · penalización aplicada")},t("reset").onclick=U,t("camera").onclick=()=>{p((S+1)%L.length)},t("inspect").onclick=()=>{window.location.href="/model-review.html"},t("inspectIntro").onclick=()=>{window.location.href="/model-review.html"},t("inspectStations").onclick=()=>{b()},t("inspectStationsIntro").onclick=()=>{b()},t("sound").onclick=()=>{c=!c,I.setEnabled(c),t("sound").textContent=c?"SONIDO ON":"SONIDO"},t("volume").oninput=a=>I.setVolume(a.target.value),t("sources").onclick=D,t("share").onclick=async()=>{const a={title:"Metro de Caracas · Línea 1",text:"Conduce el Metro de Caracas desde Caño Amarillo hasta Altamira.",url:window.location.href};try{navigator.share?await navigator.share(a):(await navigator.clipboard.writeText(window.location.href),s("Enlace copiado"))}catch(u){(u==null?void 0:u.name)!=="AbortError"&&s("Copia el enlace del navegador")}},t("completeSources").onclick=D,t("closeSources").onclick=O,t("restart").onclick=U,document.querySelectorAll("[data-a]").forEach(a=>{const u=()=>{B||e.inspection||(a.dataset.a==="throttle"&&(A=!0),a.dataset.a==="brake"&&(x=!0),a.dataset.a==="emergency"&&(m=!0,s("Freno de emergencia")))},i=()=>{a.dataset.a==="throttle"&&(A=!1),a.dataset.a==="brake"&&(x=!1)};a.onpointerdown=u,a.onpointerup=i,a.onpointercancel=i,a.onpointerleave=i});const K=((X=n.at(-1))==null?void 0:X.distance)||1;let j=performance.now(),F="";function $(a){var ee;if(document.hidden){j=a,requestAnimationFrame($);return}const u=Math.min((a-j)/1e3,.05);j=a;const i=o.tick(u,{throttle:A&&e.doorFraction<.001,brake:x,emergency:m});i.speed<=.01&&(m=!1),e.update(i.paused?0:u,i.position,{doorsOpen:i.doorsOpen,speed:i.speed,throttle:A,brake:x,emergency:m});const J=e.doorFraction<.001;I.update(u,{...i,throttle:A&&J,brake:x,emergency:m,cameraMode:L[S]});const G=n[i.target],W=Math.round(i.speed*3.6);t("speed").textContent=String(W).padStart(2,"0"),t("speedbar").style.width=Math.min(100,W/65*100)+"%",t("progress").style.width=Math.min(100,i.position/K*100)+"%",t("clock").textContent=new Date().toLocaleTimeString("es-VE",{hour12:!1}),t("next").textContent=i.complete?((ee=n.at(-1))==null?void 0:ee.name)||"—":(G==null?void 0:G.name)||"—",t("distance").textContent=i.missed?"PARADA OMITIDA · RECUPERAR":i.complete?"FIN DE LÍNEA":i.doorsOpen?`EMBARQUE · ${Math.ceil(i.dwell)} s`:`${Math.max(0,Math.round(((G==null?void 0:G.distance)||0)-i.position))} m · ${i.score} PTS`,t("doors").textContent=i.doorsOpen?"PUERTAS ABIERTAS":"PUERTAS CERRADAS",t("pause").textContent=i.paused?"CONTINUAR":"PAUSA",t("recover").style.display=i.missed?"":"none";const le=i.paused?"PAUSA":i.doorsOpen?"PUERTAS / EMBARQUE":m?"EMERGENCIA":x?"FRENANDO":A?"TRACCIÓN":"DERIVA",ce=G?i.speed*i.speed/(2*2.5):0;t("mode").textContent=`${le} · PARADA ${Math.round(ce)} m`,k.querySelectorAll(".station").forEach((ae,te)=>{ae.classList.toggle("current",te===i.target),ae.classList.toggle("passed",te<i.target)}),i.complete&&!z&&(z=!0,t("finalScore").textContent=i.score,t("complete").style.display="grid");const Z=`${i.position}:${S}:${i.doorsOpen}:${e.renderRevision}`;Z!==F&&(g.render(e.scene,e.camera),F=Z),requestAnimationFrame($)}new URLSearchParams(location.search).get("view")==="interior"&&(p(3),d()),e.update(0,o.state.position,{doorsOpen:o.state.doorsOpen,speed:o.state.speed}),g.render(e.scene,e.camera),requestAnimationFrame($),addEventListener("resize",()=>{e.resize(innerWidth,innerHeight),g.setSize(innerWidth,innerHeight),F=""})}Fe().catch(v=>console.error("Metro bootstrap failed",v));
