import{s as pe,G as ue,R as me,C as he,V as ge,O as fe,l as be,W as we,T as ve}from"./style-Db-M0n3e.js";import{R as ye,c as Ae,a as Ce,p as ne,b as Te,d as re,g as ie,e as Ee,f as oe,s as ke,h as xe}from"./station-camera-C3xaB7dO.js";import{c as Se}from"./train-doors-Jm_8N0Xa.js";const Le=`# Línea 1 del Metro de Caracas: investigación para el juego
...
`,le={color:"#d71920",realStationCount:22},X=le.color,je=[{id:"cano-amarillo",name:"Caño Amarillo",distance:0,color:X,type:"elevated",platformLayout:"side",railCenters:[0,4],interchange:null,detail:"Estación elevada junto al antiguo eje ferroviario Caracas–La Guaira, con cubierta reticulada amarilla y andenes laterales."},{id:"capitolio",name:"Capitolio",distance:900,color:X,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Conexión peatonal con El Silencio (Línea 2)",detail:"Estación profunda del centro con mezzanina y conexión peatonal hacia El Silencio."},{id:"bellas-artes",name:"Bellas Artes",distance:1800,color:X,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación profunda con andén central bajo el eje cultural de Bellas Artes."},{id:"plaza-venezuela",name:"Plaza Venezuela",distance:2800,color:X,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Líneas 3 y 4 (conexiones del complejo)",detail:"Complejo de transferencia con niveles y pasillos de conexión de alta demanda."},{id:"altamira",name:"Altamira",distance:3900,color:X,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación de tres niveles bajo la plaza y avenida Francisco de Miranda, con andén central y mezzanina."}],se=je.map((f,v)=>({...f,...pe.stations[v]}));function Ne(f,v,n,h){var H,V;const e=((H=h.train.interior)==null?void 0:H.cars)||[],d=new ue;d.name="Blender passenger lighting",f.add(d),ye.init();const j=(((V=h.lighting)==null?void 0:V.trainAreaLights)||[]).map(g=>{const I=new me(new he().fromArray(g.color),g.power/(g.width*g.height),g.width,g.height);return I.name=g.name,I.position.fromArray(g.position),I.quaternion.fromArray(g.quaternion),I.userData.carIndex=Number(g.collection.slice(-2)),I.visible=!1,d.add(I),I});let A=1,C="saloon",a=!1,T=0,l=null,E=0,u=0,k=6.63;const y=new ge(.22,2.57,0);function w(g=A,I="saloon"){A=Math.max(1,Math.min(e.length||7,Number(g)||1)),C=["saloon","seats","doors","gangway"].includes(I)?I:"saloon";const R=e[A-1];E=0,u=0,k=6.63,y.set(.22,(R==null?void 0:R.eyeY)||2.57,0),C==="seats"&&(k=4.25,y.x=-.22,y.y=2.22,E=-1.3,u=-.36),C==="doors"&&(k=2.333,y.x=.28,E=-Math.PI/2,u=-.08),C==="gangway"&&(k=-8.6,y.x=.22),T++}function S(g,I,R=g){a=I&&e.length>0;const O=e[A-1];a&&O&&(v.position.set(y.x,y.y,g+O.center+O.direction*k),v.lookAt(v.position.x+O.direction*Math.sin(E)*Math.cos(u),v.position.y+Math.sin(u),v.position.z-O.direction*Math.cos(E)*Math.cos(u)));const r=a?A:e.reduce((s,p)=>{var i;return Math.abs(g+p.center-R)<Math.abs(g+(((i=e[s-1])==null?void 0:i.center)||0)-R)?p.index:s},1);for(const s of j)s.visible=f.visible&&Math.abs(s.userData.carIndex-r)<=(a?1:0)}function N(g){k=Math.max(-9.6,Math.min(7.6,Number(g)||0)),T++}const c=n==null?void 0:n.domElement,z=g=>{!a||g.button!==0||(l={id:g.pointerId,x:g.clientX,y:g.clientY},c.setPointerCapture(g.pointerId))},P=g=>{!a||!l||l.id!==g.pointerId||(E-=(g.clientX-l.x)*.004,u=Math.max(-1.1,Math.min(1.1,u-(g.clientY-l.y)*.004)),l.x=g.clientX,l.y=g.clientY,T++)},L=()=>{l=null};return c==null||c.addEventListener("pointerdown",z),c==null||c.addEventListener("pointermove",P),c==null||c.addEventListener("pointerup",L),c==null||c.addEventListener("pointercancel",L),{select:w,update:S,move:N,cars:e,lights:j,get revision(){return T},get state(){return{carIndex:A,view:C,travel:k,enabled:a}},dispose(){c==null||c.removeEventListener("pointerdown",z),c==null||c.removeEventListener("pointermove",P),c==null||c.removeEventListener("pointerup",L),c==null||c.removeEventListener("pointercancel",L)}}}function Pe(f,v,n=[],h){if(!(h!=null&&h.train)||!(h!=null&&h.environment))throw new Error("Los modelos Blender aún no están disponibles.");const e=new f.Scene,d=Ae(e,v,h.manifest),j=Ce(e);e.fog=new f.Fog(1581354,260,900);const A=new f.Group;A.name="Caracas Metro L1 · Blender export",e.add(A);const C=h.environment;C.name="Blender environment · five stops",A.add(C),ne(C,v);const a=[];C.traverse(r=>{!r.isMesh&&r.userData.sourceCollection&&a.push(r)});const T=n.map(r=>a.find(s=>s.userData.sourceCollection===r.collection));if(T.some(r=>!r))throw new Error("Falta una estación Blender en el entorno del juego.");const l=h.train;l.name="Blender train · 7 cars",A.add(l),ne(l,v);const E=Se(l),u={forward:new f.PerspectiveCamera(70,1,.05,1200),exterior:new f.PerspectiveCamera(68,1,.1,900),platform:new f.PerspectiveCamera(68,1,.1,900),inspection:new f.PerspectiveCamera(68,1,.2,700),interior:new f.PerspectiveCamera(72,1,.035,700)},k=Ne(l,u.interior,v,h.manifest);let y="platform",w=null,S=0,N=0;const c=v?new fe(u.inspection,v.domElement):null,z=Te({camera:u.inspection,controls:c,environment:C});c&&(c.enabled=!1,c.enableDamping=!1,c.addEventListener("change",()=>{z.adjusting||(z.constrain(),S++)}));const P=()=>w?u.inspection:u[y];function L(r){return y=["forward","exterior","platform","interior"].includes(r)?r:"platform",S++,P()}function H(r=N,s="platform"){r=Math.max(0,Math.min(n.length-1,Number(r)||0));const p=ie(r,s,u.inspection.aspect);w={index:r,...p};for(const b of a)b.visible=p.collections.includes(b.userData.sourceCollection);re(v,p),j.show(r,p.kind);const i=n[r].distance;return l.visible=["platform","detail","concourse"].includes(p.kind)&&l.position.z>=i-148&&l.position.z<=i+150,c&&(c.enabled=!0),z.setView(r,p),d.focus(u.inspection.position,p.collection,p.exterior,["plan","section"].includes(p.kind)),S++,w}function V(){w=null,re(v,{}),j.hide(),c&&(c.enabled=!1),z.clear(),a.forEach(r=>{r.visible=!0}),l.visible=!0,S++}function g(r){(w==null?void 0:w.kind)!=="tunnel"||!Number.isFinite(Number(r))||(z.moveTunnel(r),d.focus(u.inspection.position,w.collection),S++)}function I(r=.016,s=0,p={}){l.position.set(0,0,Number(s)||0),E.update(r,p);const i=l.position.z;u.forward.position.set(0,2.5,i+5),u.forward.lookAt(0,2,i+15);const b=k.cars[k.state.carIndex-1],B=y==="interior"&&b?i+b.center+b.direction*k.state.travel:y==="platform"?i-84:i+4.6;N=n.findIndex(x=>B<=x.distance+5),N<0&&(N=n.length-1);const _=n[N],F=(x,M)=>x>=M.distance-144&&x<=M.distance+4,D=F(B,_),$=["Bellas Artes","Altamira"].includes(_==null?void 0:_.name),q=$?2.5:(_==null?void 0:_.name)==="Capitolio"?-2.65:-3.9;u.platform.position.set(D?q:0,2.73,D?i-84:i+5),u.platform.lookAt(D?q:0,2.98,D?i-69:i+23);const Y=n.some(x=>F(i+4.6,x));if(u.exterior.position.set(Y?$?7.5:q:0,2.73,i+4.6),u.exterior.lookAt(0,2.1,Y?i-4:i+22),k.update(i,y==="interior"&&!w,P().position.z),w)z.constrain(),d.focus(u.inspection.position,w.collection,w.exterior,["plan","section"].includes(w.kind));else{const x=P().position.clone(),M=n.find(K=>x.z>=K.distance-145&&x.z<=K.distance+5),U=Math.max(0,n.findLastIndex(K=>x.z>K.distance+5));d.focus(x,(M==null?void 0:M.collection)||`Tunnel ${n[U].id}`)}}function R(r,s){if(Object.values(u).forEach(p=>{p.aspect=r/Math.max(1,s),p.updateProjectionMatrix()}),(w==null?void 0:w.kind)==="plan"){const{index:p}=w,i=ie(p,"plan",u.inspection.aspect);w={index:p,...i},z.setView(p,i)}else z.constrain();S++}function O(){A.traverse(r=>{var s;r.geometry&&r.geometry.dispose(),(s=r.material)!=null&&s.dispose&&r.material.dispose()}),c==null||c.dispose(),z.dispose(),d.dispose(),k.dispose(),j.dispose()}return{scene:e,root:A,train:l,cameras:u,get camera(){return P()},get inspection(){return w},get activeStation(){return N},get renderRevision(){return S+k.revision+E.revision},get doorFraction(){return E.fraction},doors:E,setCameraMode:L,inspectStation:H,leaveInspection:V,moveTunnel:g,update:I,resize:R,dispose:O,controls:c,stationAssemblies:T,railPaths:[],lighting:d,interior:k,cameraGuard:z,assetSource:h.source,manifest:h.manifest}}function _e(f=[],{routeEnd:v=1/0}={}){var A,C;const n=((A=f[0])==null?void 0:A.distance)??0,h=Math.max(((C=f.at(-1))==null?void 0:C.distance)??n,v);let e;const d=()=>{var a;return{position:n,speed:0,target:0,doorsOpen:!0,doorSide:((a=f[0])==null?void 0:a.platformLayout)==="island"?1:-1,doorStation:0,lastServed:-1,dwell:3,started:!1,missed:!1,complete:!1,paused:!1,score:100,serviceCount:0}};return e=d(),{get state(){return{...e}},start(){return e.started=!0,this},reset(){return e=d(),e.started=!0,this},pause(a=!e.paused){return e.paused=a,this},setDoors(a){if(a=!!a,a===e.doorsOpen)return this.state;const T=e.doorsOpen?e.doorStation:f.findIndex((E,u)=>(u===e.target||u===e.lastServed)&&Math.abs(e.position-E.distance)<=12),l=f[T];return e.paused||e.speed>.04||!l||Math.abs(e.position-l.distance)>12||e.missed||e.complete?this.state:(a?(e.doorsOpen=!0,e.doorStation=T,e.doorSide=l.platformLayout==="island"?1:-1,e.dwell=T>e.lastServed?3:0):e.dwell<=0&&(e.doorsOpen=!1,T>e.lastServed&&(e.serviceCount++,e.lastServed=T,e.target===f.length-1?e.complete=!0:e.target++)),this.state)},toggleDoors(){return this.setDoors(!e.doorsOpen)},recover(){const a=f[e.target];return!a||e.complete||!e.missed?this.state:(e.position=a.distance,e.speed=0,e.doorsOpen=!0,e.doorStation=e.target,e.doorSide=a.platformLayout==="island"?1:-1,e.dwell=3,e.missed=!1,e.score=Math.max(0,e.score-15),this.state)},tick(a,T={}){if(!e.started||e.paused||e.complete)return this.state;const l=Math.min(Math.max(Number(a)||0,0),.05);e.doorsOpen&&(e.dwell=Math.max(0,e.dwell-l));const E=!!T.throttle&&!e.doorsOpen&&!e.missed;T.emergency?e.speed=Math.max(0,e.speed-3*l):T.brake?e.speed=Math.max(0,e.speed-2.5*l):E?e.speed=Math.min(18,e.speed+1.15*l):e.speed=Math.max(0,e.speed-.22*l),e.position+=e.speed*l,e.position>=h&&(e.position=h,e.speed=0);const u=f[e.target];return u&&e.position>u.distance+12&&!e.missed&&(e.missed=!0,e.score=Math.max(0,e.score-20)),this.state}}}const Oe=[{title:"Metro De Caracas Compilación Trenes Alstom y CAF",url:"https://www.youtube.com/watch?v=Ej0X90zrtNg",creator:"Dani215 Railway",date:"2024-07-17",license:"No redistributable licence verified on the source page; reference-only",authenticity:"creator's description identifies Line 1 CAF Renfe Serie 6 footage (61047–61080), alongside Alstom footage; no audio copied"},{title:"OpenBVE Venezuela CAF Serie 6 asset pack (external download)",url:"https://www.mediafire.com/file/om2u9qlgt7gaahw/Pack_Series_10000_y_60000_MCCS_FINAL.zip/file",creator:"OpenBVE Venezuela / uploader credited on the pack page",license:"No reuse licence or permission statement found in the page/archive; do not bundle",authenticity:"CAF S6 train asset includes WAV filenames, but the archive does not establish that they are field recordings"},{title:"Caracas Metro compilation (Line 1, 2006–2013)",url:"https://www.youtube.com/watch?v=86YCTUO5gQw",creator:"MonteBRujaFM",license:"No redistributable licence verified on the source page; reference-only",authenticity:"creator describes Caracas Line 1 rolling-stock footage; audio remains copyrighted"},{title:"Caracas Metro 10000 and 20000 series sound examples",url:"https://www.youtube.com/watch?v=gtgx51bclew",creator:"YouTube uploader (linked by r/transit discussion)",license:"No redistributable licence verified on the source page; reference-only",authenticity:"identified as older Caracas Alstom 10000/20000-series stock, not CAF 60000-series; not bundled"},{title:"Caracas Metro 10000 and 20000 series sound example 2",url:"https://www.youtube.com/watch?v=cbB9gqDMQ0o",creator:"YouTube uploader (linked by r/transit discussion)",license:"No redistributable licence verified on the source page; reference-only",authenticity:"identified as older Caracas Alstom 10000/20000-series stock, not CAF 60000-series; not bundled"}],Be={authenticity:"procedural gameplay fallback; no bundled Caracas recording"},Q=(f,v=0,n=1)=>Math.max(v,Math.min(n,f));function Fe(f,v=2){const n=f.createBuffer(1,f.sampleRate*v,f.sampleRate),h=n.getChannelData(0);let e=0;for(let d=0;d<h.length;d+=1)e=e*.965+(Math.random()*2-1)*.18,h[d]=e;return n}function De({now:f=()=>performance.now()/1e3,contextFactory:v=null}={}){let n=null,h=!1,e=.65,d=!1,j=null,A=f(),C=0,a=0,T=[],l=null,E=null,u=[],k=[],y=null,w=null,S=null,N=null,c=null;function z(r,s,p,i=0){const b=n.createOscillator(),B=n.createGain();return b.type=r,b.frequency.value=s,B.gain.value=i,b.connect(B).connect(p),b.start(),T.push(b,B),{osc:b,gain:B}}function P(){if(n||d)return;const r=v||globalThis.AudioContext||globalThis.webkitAudioContext;if(!r)return;n=v?v():new r,l=n.createGain(),l.gain.value=0,E=n.createDynamicsCompressor(),E.threshold.value=-18,E.knee.value=16,E.ratio.value=7,E.attack.value=.008,E.release.value=.18,l.connect(E).connect(n.destination),u=[56,112,168].map((D,$)=>{const q=z($===1?"triangle":"sine",D,l,0);return k.push(q.gain),q.osc});const s=n.createBufferSource();s.buffer=Fe(n),s.loop=!0;const p=n.createBiquadFilter();p.type="lowpass",p.frequency.value=760,p.Q.value=.45,y=n.createGain(),y.gain.value=0,s.connect(p).connect(y).connect(l),s.start(),T.push(s,p,y),w=n.createGain(),w.gain.value=0;const i=z("sawtooth",145,w,.07),b=n.createBiquadFilter();b.type="bandpass",b.frequency.value=820,b.Q.value=2.1,w.disconnect(),w.connect(b).connect(l),T.push(b,i),S=n.createGain(),S.gain.value=.002,S.connect(l);const B=z("sine",47,S,.6);T.push(B),N=n.createGain(),N.gain.value=0;const _=n.createOscillator(),F=n.createBiquadFilter();_.type="sawtooth",_.frequency.value=1700,F.type="bandpass",F.frequency.value=1350,F.Q.value=2,_.connect(F).connect(N).connect(l),_.start(),T.push(_,F,N),c=n.createGain(),c.gain.value=1,c.connect(l),T.push(c)}const L=(r,s,p=.06)=>{if(!n||!r)return;const i=n.currentTime;r.cancelScheduledValues(i),r.setTargetAtTime(s,i,p)};function H(){if(!n||!h||!N)return;const r=n.currentTime;[660,880].forEach((s,p)=>{const i=n.createOscillator(),b=n.createGain();i.type="sine",i.frequency.value=s,b.gain.setValueAtTime(1e-4,r+p*.16),b.gain.exponentialRampToValueAtTime(.06,r+p*.16+.012),b.gain.exponentialRampToValueAtTime(1e-4,r+p*.16+.22),i.connect(b).connect(c),i.start(r+p*.16),i.stop(r+p*.16+.25)})}function V(r,s={}){const p=Q(Number(r)||0,0,.1);A=f();const i=!!s.doorsOpen;if(j!==i&&j!==null&&(j===!0&&i===!1&&H(),a=1),j=i,!n||!h||d)return;const b=Q((Number(s.speed)||0)/18),B=!!(s.brake||s.emergency);a=Math.max(0,a-p/.8);const F=s.camera==="exterior"||s.cameraMode==="exterior"?.68:1,D=!!s.started&&!s.paused&&!s.complete&&!i,$=!!s.started&&!s.paused&&!s.complete,q=D?F:0;L(l.gain,e*($?F*(i?.055:.1375+b*.275):0),.08),u.forEach((x,M)=>L(x.frequency,[56,113,171][M]+b*[130,260,390][M],.1));const Y=s.throttle?1:.35;if(k.forEach((x,M)=>L(x.gain,q*(.018-M*.003)*(.15+b)*Y,.1)),L(y.gain,q*(.018+b*.035),.14),L(w.gain,q*(B?.8:0)*b,.09),L(S.gain,$?F*.012:0,.2),L(N.gain,a*.012,.08),C+=p*(1.5+b*13),D&&b>.03&&C>=1){C%=1;const x=n.createOscillator(),M=n.createGain(),U=n.currentTime;x.type="triangle",x.frequency.value=540+Math.random()*90,M.gain.setValueAtTime(1e-4,U),M.gain.exponentialRampToValueAtTime(Math.max(2e-4,.018*b),U+.004),M.gain.exponentialRampToValueAtTime(1e-4,U+.045),x.connect(M).connect(l),x.start(U),x.stop(U+.05)}}async function g(r){return d?!1:(h=!!r,h&&(P(),n&&n.state==="suspended"&&n.resume&&typeof n.startRendering!="function"&&await n.resume()),n&&L(l.gain,h?e*.001:0,.04),h)}function I(){j=null,C=0,a=0,n&&L(l.gain,0,.04)}function R(r){return e=Q(Number(r)/100,0,1),n&&h&&L(l.gain,e*.001,.04),e*100}function O(){var r;d=!0,h=!1,T.forEach(s=>{var p,i;try{(p=s.stop)==null||p.call(s),(i=s.disconnect)==null||i.call(s)}catch{}}),(r=n==null?void 0:n.close)==null||r.call(n),T=[],n=null}return{setEnabled:g,setVolume:R,update:V,reset:I,dispose:O,status:()=>({enabled:h,available:!!n,running:!!(n&&n.state==="running"),lastUpdate:A})}}async function qe(){var K;const f=document.querySelector("#app");f.innerHTML='<div class="asset-loading" role="status"><div class="asset-loading-mark">M</div><div class="asset-loading-title">Cargando exportación Blender</div><div class="asset-loading-detail" id="assetLoadingDetail">Validando manifest.json…</div></div>';let v;try{v=await be({onProgress:(t,m)=>{const o=document.querySelector("#assetLoadingDetail");o&&(o.textContent=`${t} · ${Math.round(m*100)}%`)}})}catch(t){const m=String((t==null?void 0:t.message)||t);throw f.innerHTML=`<div class="asset-error" role="alert"><div class="asset-loading-mark">!</div><h1>No se pudo abrir el mundo Blender</h1><p>${m.replace(/[&<>]/g,o=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[o])}</p><p>Comprueba que <code>public/models/blender/manifest.json</code>, <code>train.glb</code> y <code>environment.glb</code> estén publicados y vuelve a cargar.</p><button onclick="location.reload()">REINTENTAR</button></div>`,t}const n=(Array.isArray(se)?se:[]).map((t,m)=>{var o;return{...t,...v.manifest.stations[m]||{},distance:Number(((o=v.manifest.stations[m])==null?void 0:o.distance)??t.distance)}});f.replaceChildren();const h=new we({antialias:!0,powerPreference:"low-power"});Ee(h),h.setSize(innerWidth,innerHeight),f.append(h.domElement);const e=Pe(ve,h,n,v);e.resize(innerWidth,innerHeight);const d=_e(n,{routeEnd:oe(n.length-1).max-5}),j=De();window.__metro={world:e,sim:d,renderer:h,assetSource:v.source,manifest:v.manifest,assetsReady:!0};const A=document.createElement("div");A.className="ui",A.innerHTML=`
