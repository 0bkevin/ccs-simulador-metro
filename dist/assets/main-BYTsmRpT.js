import"./modulepreload-polyfill-B5Qt9EMX.js";import{s as be,O as we,l as ve,W as ye,T as Ae}from"./style-2FX8oNIw.js";import{c as Ce,a as Te,p as ce,b as Ee,d as pe,g as ue,e as xe,f as me,s as Se,h as ke}from"./station-camera-BFjsLpJf.js";import{c as Re,a as Le,b as ze,r as Ie}from"./train-doors-BhD7l2Jb.js";import{c as Pe,a as je,b as Me,r as Ne}from"./audio-009sZ_ss.js";const Oe=`# Línea 1 del Metro de Caracas: investigación para el juego
...
`,_e=`# Línea 1 station references (five-station game route)
...
`,Fe=`# Independent station and tunnel study · 7 September 2026
...
`,Be=`# CAF Serie 6 exterior and door controls
...
`,ge={color:"#d71920",realStationCount:22},Q=ge.color,qe=[{id:"cano-amarillo",name:"Caño Amarillo",distance:0,color:Q,type:"elevated",platformLayout:"side",railCenters:[0,4],interchange:null,detail:"Estación elevada junto al antiguo eje ferroviario Caracas–La Guaira, con cubierta reticulada amarilla y andenes laterales."},{id:"capitolio",name:"Capitolio",distance:900,color:Q,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Conexión peatonal con El Silencio (Línea 2)",detail:"Estación profunda del centro con mezzanina y conexión peatonal hacia El Silencio."},{id:"bellas-artes",name:"Bellas Artes",distance:1800,color:Q,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación profunda con andén central bajo el eje cultural de Bellas Artes."},{id:"plaza-venezuela",name:"Plaza Venezuela",distance:2800,color:Q,type:"subterránea",platformLayout:"side",railCenters:[0,4],interchange:"Líneas 3 y 4 (conexiones del complejo)",detail:"Complejo de transferencia con niveles y pasillos de conexión de alta demanda."},{id:"altamira",name:"Altamira",distance:3900,color:Q,type:"subterránea",platformLayout:"island",railCenters:[0,10],interchange:null,detail:"Estación de tres niveles bajo la plaza y avenida Francisco de Miranda, con andén central y mezzanina."}],he=qe.map((p,v)=>({...p,...be.stations[v]}));function Ve(p,v,u=[],h){if(!(h!=null&&h.train)||!(h!=null&&h.environment))throw new Error("Los modelos Blender aún no están disponibles.");const a=new p.Scene,s=Ce(a,v,h.manifest),P=Te(a);a.fog=new p.Fog(1581354,260,900);const y=new p.Group;y.name="Caracas Metro L1 · Blender export",a.add(y);const A=h.environment;A.name="Blender environment · five stops",y.add(A),ce(A,v);const t=[];A.traverse(o=>{!o.isMesh&&o.userData.sourceCollection&&t.push(o)});const x=u.map(o=>t.find(C=>C.userData.sourceCollection===o.collection));if(x.some(o=>!o))throw new Error("Falta una estación Blender en el entorno del juego.");const c=h.train;c.name="Blender train · 7 cars",y.add(c),ce(c,v),c.traverse(o=>{if(o.isMesh){const C=Array.isArray(o.material)?o.material:[o.material];o.castShadow=C.some(m=>!m.transparent)}});const S=Re(c,Ie),l={forward:new p.PerspectiveCamera(70,1,.05,1200),exterior:new p.PerspectiveCamera(68,1,.1,900),platform:new p.PerspectiveCamera(68,1,.1,900),inspection:new p.PerspectiveCamera(68,1,.2,700),interior:new p.PerspectiveCamera(72,1,.035,700)},O=Le(c,l.interior,v,h.manifest),W=ze(c);let E="platform",d=null,b=0,z=0;const f=v?new we(l.inspection,v.domElement):null,j=Ee({camera:l.inspection,controls:f,environment:A});f&&(f.enabled=!1,f.enableDamping=!1,f.addEventListener("change",()=>{j.adjusting||(j.constrain(),b++)}));const M=()=>d?l.inspection:l[E];function Z(o){return E=["forward","exterior","platform","interior"].includes(o)?o:"platform",b++,M()}function ee(o=z,C="platform"){o=Math.max(0,Math.min(u.length-1,Number(o)||0));const m=ue(o,C,l.inspection.aspect);d={index:o,...m};for(const T of t)T.visible=m.collections.includes(T.userData.sourceCollection);pe(v,m),P.show(o,m.kind);const g=u[o].distance;return c.visible=["platform","detail","concourse"].includes(m.kind)&&c.position.z>=g-148&&c.position.z<=g+150,f&&(f.enabled=!0),j.setView(o,m),s.focus(l.inspection.position,m.collection,m.exterior,["plan","section"].includes(m.kind)),b++,d}function I(){d=null,pe(v,{}),P.hide(),f&&(f.enabled=!1),j.clear(),t.forEach(o=>{o.visible=!0}),c.visible=!0,b++}function ae(o){(d==null?void 0:d.kind)!=="tunnel"||!Number.isFinite(Number(o))||(j.moveTunnel(o),s.focus(l.inspection.position,d.collection),b++)}function te(o=.016,C=0,m={}){c.position.set(0,0,Number(C)||0),S.update(o,m),W.update(m,S.fraction);const g=c.position.z;l.forward.position.set(0,2.5,g+5),l.forward.lookAt(0,2,g+15);const T=O.cars[O.state.carIndex-1],V=E==="interior"&&T?g+T.center+T.direction*O.state.travel:E==="platform"?g-84:g+4.6;z=u.findIndex(R=>V<=R.distance+5),z<0&&(z=u.length-1);const k=u[z],U=(R,G)=>R>=G.distance-144&&R<=G.distance+4,F=U(V,k),w=["Bellas Artes","Altamira"].includes(k==null?void 0:k.name),D=w?2.5:(k==null?void 0:k.name)==="Capitolio"?-2.65:-3.9;l.platform.position.set(F?D:0,2.73,F?g-84:g+5),l.platform.lookAt(F?D:0,2.98,F?g-69:g+23);const J=u.some(R=>U(g+4.6,R));if(l.exterior.position.set(J?w?7.5:D:0,2.73,g+4.6),l.exterior.lookAt(0,2.1,J?g-4:g+22),O.update(g,E==="interior"&&!d,M().position.z),d)j.constrain(),s.focus(l.inspection.position,d.collection,d.exterior,["plan","section"].includes(d.kind));else{const R=M().position.clone(),G=u.find(K=>R.z>=K.distance-145&&R.z<=K.distance+5),Y=Math.max(0,u.findLastIndex(K=>R.z>K.distance+5)),ne=E==="interior"&&!["operator","cab-seat"].includes(O.state.view);s.focus(R,(G==null?void 0:G.collection)||`Tunnel ${u[Y].id}`,!1,!1,ne)}}function _(o,C){if(Object.values(l).forEach(m=>{m.aspect=o/Math.max(1,C),m.updateProjectionMatrix()}),(d==null?void 0:d.kind)==="plan"){const{index:m}=d,g=ue(m,"plan",l.inspection.aspect);d={index:m,...g},j.setView(m,g)}else j.constrain();b++}function X(){y.traverse(o=>{var C;o.geometry&&o.geometry.dispose(),(C=o.material)!=null&&C.dispose&&o.material.dispose()}),f==null||f.dispose(),j.dispose(),s.dispose(),O.dispose(),W.dispose(),P.dispose()}return{scene:a,root:y,train:c,cameras:l,get camera(){return M()},get inspection(){return d},get activeStation(){return z},get renderRevision(){return b+O.revision+S.revision+W.revision},get doorFraction(){return S.fraction},doors:S,setCameraMode:Z,inspectStation:ee,leaveInspection:I,moveTunnel:ae,update:te,resize:_,dispose:X,controls:f,stationAssemblies:x,railPaths:[],lighting:s,interior:O,cab:W,cameraGuard:j,assetSource:h.source,manifest:h.manifest}}function Ue(p=[],{routeEnd:v=1/0}={}){var y,A;const u=((y=p[0])==null?void 0:y.distance)??0,h=Math.max(((A=p.at(-1))==null?void 0:A.distance)??u,v);let a;const s=()=>{var t;return{position:u,speed:0,target:0,doorsOpen:!0,doorSide:((t=p[0])==null?void 0:t.platformLayout)==="island"?1:-1,doorStation:0,lastServed:-1,dwell:3,started:!1,missed:!1,complete:!1,paused:!1,score:100,serviceCount:0}};return a=s(),{get state(){return{...a}},start(){return a.started=!0,this},reset(){return a=s(),a.started=!0,this},pause(t=!a.paused){return a.paused=t,this},setDoors(t){if(t=!!t,t===a.doorsOpen)return this.state;const x=a.doorsOpen?a.doorStation:p.findIndex((S,l)=>(l===a.target||l===a.lastServed)&&Math.abs(a.position-S.distance)<=12),c=p[x];return a.paused||a.speed>.04||!c||Math.abs(a.position-c.distance)>12||a.missed||a.complete?this.state:(t?(a.doorsOpen=!0,a.doorStation=x,a.doorSide=c.platformLayout==="island"?1:-1,a.dwell=x>a.lastServed?3:0):a.dwell<=0&&(a.doorsOpen=!1,x>a.lastServed&&(a.serviceCount++,a.lastServed=x,a.target===p.length-1?a.complete=!0:a.target++)),this.state)},toggleDoors(){return this.setDoors(!a.doorsOpen)},recover(){const t=p[a.target];return!t||a.complete||!a.missed?this.state:(a.position=t.distance,a.speed=0,a.doorsOpen=!0,a.doorStation=a.target,a.doorSide=t.platformLayout==="island"?1:-1,a.dwell=3,a.missed=!1,a.score=Math.max(0,a.score-15),this.state)},tick(t,x={}){if(!a.started||a.paused||a.complete)return this.state;const c=Math.min(Math.max(Number(t)||0,0),.05);a.doorsOpen&&(a.dwell=Math.max(0,a.dwell-c));const S=!!x.throttle&&!a.doorsOpen&&!a.missed;x.emergency?a.speed=Math.max(0,a.speed-3*c):x.brake?a.speed=Math.max(0,a.speed-2.5*c):S?a.speed=Math.min(18,a.speed+1.15*c):a.speed=Math.max(0,a.speed-.22*c),a.position+=a.speed*c,a.position>=h&&(a.position=h,a.speed=0);const l=p[a.target];return l&&a.position>l.distance+12&&!a.missed&&(a.missed=!0,a.score=Math.max(0,a.score-20)),this.state}}}async function Ge(){var ie;const p=document.querySelector("#app");p.innerHTML='<div class="asset-loading" role="status"><div class="asset-loading-mark">M</div><div class="asset-loading-title">Cargando exportación Blender</div><div class="asset-loading-detail" id="assetLoadingDetail">Validando manifest.json…</div></div>';let v;try{v=await ve({onProgress:(e,n)=>{const r=document.querySelector("#assetLoadingDetail");r&&(r.textContent=`${e} · ${Math.round(n*100)}%`)}})}catch(e){const n=String((e==null?void 0:e.message)||e);throw p.innerHTML=`<div class="asset-error" role="alert"><div class="asset-loading-mark">!</div><h1>No se pudo abrir el mundo Blender</h1><p>${n.replace(/[&<>]/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[r])}</p><p>Comprueba que <code>public/models/blender/manifest.json</code>, <code>train.glb</code> y <code>environment.glb</code> estén publicados y vuelve a cargar.</p><button onclick="location.reload()">REINTENTAR</button></div>`,e}const u=(Array.isArray(he)?he:[]).map((e,n)=>{var r;return{...e,...v.manifest.stations[n]||{},distance:Number(((r=v.manifest.stations[n])==null?void 0:r.distance)??e.distance)}});p.replaceChildren();const h=new ye({antialias:!0,powerPreference:"low-power"});xe(h),h.setSize(innerWidth,innerHeight),p.append(h.domElement);const a=Ve(Ae,h,u,v);a.resize(innerWidth,innerHeight);const s=Ue(u,{routeEnd:me(u.length-1).max-5}),P=Pe({stops:u,onStatus:ee});window.__metro={world:a,sim:s,audio:P,renderer:h,assetSource:v.source,manifest:v.manifest,assetsReady:!0};const y=document.createElement("div");y.className="ui",y.innerHTML=`
...
<div class="overlay" id="sourceModal" style="display:none"><div class="card source-card"><button class="corner" id="closeSources">CERRAR ×</button><div class="eyebrow">Documentación</div><h1>Fuentes</h1><p>Investigación sobre diseño, estaciones, trenes y mecánica de la Línea 1. Fuentes primarias consultadas:</p><div class="source-links"><a href="https://openjicareport.jica.go.jp/pdf/11789237_03.pdf" target="_blank" rel="noreferrer">JICA · datos de línea</a><a href="https://www.aschinfraestructuras.com/linea-caracas" target="_blank" rel="noreferrer">ASCH · rehabilitación de vía</a><a href="https://admin.cafmobility.com/uploads/281_CAF_Catalogo_General_ES_601604d06c.pdf" target="_blank" rel="noreferrer">CAF · catálogo</a><a href="https://www.revistaitransporte.es/wp-content/uploads/2016/02/2013_49.pdf" target="_blank" rel="noreferrer">INECO · tren CAF y rehabilitación</a><a href="https://www.alstom.com/fr/press-releases-news/2005/9/ALSTOM-remporte-un-contrat-cle-en-main-pour-le-Metro-de-Caracas-au-Venezuela-20050916" target="_blank" rel="noreferrer">Alstom · Metro de Caracas</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery1.htm" target="_blank" rel="noreferrer">UrbanRail · Altamira y Bellas Artes</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery2.htm" target="_blank" rel="noreferrer">UrbanRail · Capitolio</a><a href="https://www.urbanrail.net/am/cara/pix/caracas-gallery4.htm" target="_blank" rel="noreferrer">UrbanRail · Plaza Venezuela</a><a href="https://fundaayc.com/2024/07/14/algo-mas-sobre-la-postal-no-411/" target="_blank" rel="noreferrer">Fundación Arquitectura y Ciudad · Altamira</a><a href="https://recyt.fecyt.es/index.php/CyTET/article/download/83795/61863/276024" target="_blank" rel="noreferrer">Bemergui · arquitectura de estaciones</a></div><p><small>El audio utiliza fragmentos atribuidos a videos del Metro de Caracas. Consulta aquí cada grabación y su fuente.</small></p><button class="secondary" id="realListen">VER GRABACIÓN CAF/ALSTOM</button><iframe id="realFrame" title="Referencia real del Metro de Caracas" style="display:none;width:100%;aspect-ratio:16/9;border:0;margin-top:1rem" allow="autoplay; encrypted-media" allowfullscreen></iframe><div id="audioSources"></div><pre id="researchText"></pre><pre id="stationResearchText"></pre></div></div>`,p.append(y);const A=document.createElement("section");A.className="station-review-ui in-game-stations",A.hidden=!0,A.setAttribute("aria-label","Explorar estaciones"),A.innerHTML='<button id="returnToGame" class="back-link">← VOLVER A CONDUCIR</button><div class="review-kicker">EXPLORAR ESTACIÓN · JUEGO EN PAUSA</div><h1 id="gameStationTitle"></h1><p id="gameStationDetail"></p><label>ESTACIÓN <select id="gameStationSelect" aria-label="Estación para explorar"></select></label><div class="review-actions"><button data-station-view="platform">ANDÉN</button><button data-station-view="detail">DETALLE</button><button data-station-view="concourse">MEZZANINA</button><button data-station-view="entrance">ACCESO</button><button data-station-view="south">PLAZA SUR</button><button data-station-view="plan">PLANTA</button><button data-station-view="section">CORTE</button><button data-station-view="tunnel">TÚNEL</button></div><p id="gameStationMetrics" class="station-metrics"></p><label id="gameTunnelTravelLabel" hidden>RECORRER TÚNEL <input id="gameTunnelTravel" type="range" step="1" aria-label="Recorrer túnel en metros"></label><p class="review-hint">Planta y corte: geometría del modelo en metros; medidas de estación estimadas.</p><p class="review-hint">Arrastra para mirar · rueda para acercarte.<br>Tu recorrido se conserva. Esc para volver.</p>',y.append(A);const t=e=>y.querySelector("#"+e),x=t("stationList");u.forEach((e,n)=>{const r=document.createElement("div");r.className="station",r.textContent=e.name,r.dataset.i=n,x.append(r)}),t("researchText").textContent=Oe,t("stationResearchText").textContent=Fe+`

`+_e;const c=document.createElement("pre");c.textContent=`${Be}

${De}`,t("stationResearchText").before(c);const S=document.createElement("a");S.href="https://commons.wikimedia.org/wiki/File:Metro_de_caracas_linea_1.jpg",S.target="_blank",S.rel="noopener noreferrer",S.textContent="CAF Serie 6 · fotografía original usada para el modelo",y.querySelector(".source-links").prepend(S),t("audioSources").textContent=je.authenticity,Me.forEach(e=>{const n=document.createElement("a");n.href=e.url,n.target="_blank",n.rel="noopener noreferrer",n.textContent=`${e.title} — ${e.creator}`,n.style.display="block",t("audioSources").append(n)});const l=document.createElement("details"),O=document.createElement("summary");O.textContent="ESCUCHAR LOS 17 FRAGMENTOS DEL JUEGO",l.append(O);const W={idle:"Tren detenido",rolling:"Rodadura en túnel",traction:"Salida del CAF",braking:"Frenado del CAF","brake-release":"Final de la parada","doors-open":"Apertura de puertas","doors-close":"Aviso y cierre de puertas"};for(const[e,n]of Object.entries(Ne.clips)){const r=u.find($=>e.endsWith($.id)),i=W[e]||`${e.startsWith("arrival-")?"Anuncio":"Ambiente"} · ${(r==null?void 0:r.name)||e}`,L=document.createElement("div");L.className="audio-recording";const N=document.createElement("div");N.textContent=i;const q=document.createElement("audio");q.controls=!0,q.preload="none",q.src=n.url,q.setAttribute("aria-label",i),q.addEventListener("play",()=>{l.querySelectorAll("audio").forEach($=>{$!==q&&$.pause()}),t("realFrame").src="",t("realFrame").style.display="none"});const H=document.createElement("a");H.href=`https://www.youtube.com/watch?v=${n.source}&t=${Math.floor(n.start)}s`,H.target="_blank",H.rel="noopener noreferrer",H.textContent="Ver fragmento original ↗",L.append(N,q,H),l.append(L)}t("audioSources").append(l),t("realListen").onclick=()=>{l.querySelectorAll("audio").forEach(e=>e.pause()),t("realFrame").src="https://www.youtube-nocookie.com/embed/Ej0X90zrtNg",t("realFrame").style.display="block"};let E=!1,d=!1,b=!1,z=0;const f=["platform","exterior","forward","interior"],j={platform:"ANDÉN",exterior:"EXTERIOR",forward:"MARCHA",interior:"INTERIOR"};t("camera").textContent="VISTA: ANDÉN";let M=!1,Z=0;function ee(e){M=e.enabled,t("sound").textContent=M?e.loading?"CARGANDO AUDIO…":"SONIDO ON":"SONIDO",t("sound").setAttribute("aria-pressed",String(M))}function I(e=0,n=s.state){P.update(e,{...n,hidden:document.hidden,doorFraction:a.doorFraction,doorAudio:a.doors.audio,throttle:E&&a.doorFraction<.001,brake:d,emergency:b,cameraMode:f[z]})}function ae(){s.pause(),B=performance.now(),I(),w(s.state.paused?"Pausa":"Continuar")}let te=!1,_=!1,X=!1,o=!1;const C=t("gameStationSelect");u.forEach((e,n)=>{const r=document.createElement("option");r.value=n,r.textContent=e.name,C.append(r)});function m(e=a.activeStation,n="platform"){a.inspection||(o=s.state.paused,D(!1),b=!1,s.pause(!0),I());const r=a.inspectStation(e,n),i=Se[r.index];C.value=r.index,t("gameStationTitle").textContent=i.name,t("gameStationDetail").textContent=i.detail,t("gameStationMetrics").textContent=ke(r.index,r.kind),t("gameTunnelTravelLabel").hidden=r.kind!=="tunnel";const L=me(r.index);t("gameTunnelTravel").min=L.min,t("gameTunnelTravel").max=L.max,t("gameTunnelTravel").value=a.camera.position.z,A.querySelectorAll("[data-station-view]").forEach(N=>{N.hidden=!r.views.includes(N.dataset.stationView),N.classList.toggle("active",N.dataset.stationView===r.kind),N.setAttribute("aria-pressed",N.dataset.stationView===r.kind)}),A.hidden=!1,T.hidden=!0,y.classList.add("station-inspecting")}function g(){a.leaveInspection(),A.hidden=!0,y.classList.remove("station-inspecting"),T.hidden=f[z]!=="interior",s.pause(o),B=performance.now(),I()}C.onchange=()=>m(Number(C.value)),A.querySelectorAll("[data-station-view]").forEach(e=>{e.onclick=()=>m(a.inspection.index,e.dataset.stationView)}),t("returnToGame").onclick=g,t("gameTunnelTravel").oninput=e=>{a.moveTunnel(e.target.value),e.target.value=a.camera.position.z};const T=document.createElement("section");T.className="interior-controls",T.hidden=!0,T.setAttribute("aria-label","Interior del tren"),T.innerHTML='<div class="route-label">Interior · CAF Serie 6</div><label>COCHE <select id="interiorCar" aria-label="Coche del tren"></select></label><div class="interior-presets"><button data-interior-view="saloon">SALÓN</button><button data-interior-view="seats">ASIENTOS</button><button data-interior-view="doors">PUERTAS</button><button data-interior-view="gangway">INTERCONEXIÓN</button><button data-interior-view="operator">PUESTO DEL OPERADOR</button><button data-interior-view="cab-seat">ASIENTO DEL OPERADOR</button></div><label id="interiorTravelLabel">RECORRER COCHE <input id="interiorTravel" type="range" min="-9.6" max="7.6" step=".01" value="6.63" aria-label="Posición dentro del coche"></label><p>Arrastra para mirar alrededor.<br>W / S controlan el tren · E abre o cierra puertas.</p>',y.append(T);for(let e=1;e<=7;e++){const n=document.createElement("option");n.value=e,n.textContent=`${e} · ${e===1||e===7?"Extremo":"Intermedio"}`,t("interiorCar").append(n)}function V(e="saloon"){a.interior.select(Number(t("interiorCar").value),e),t("interiorCar").value=a.interior.state.carIndex,t("interiorTravelLabel").hidden=["operator","cab-seat"].includes(e),t("interiorTravel").value=a.interior.state.travel,T.querySelectorAll("[data-interior-view]").forEach(n=>{n.classList.toggle("active",n.dataset.interiorView===e),n.setAttribute("aria-pressed",n.dataset.interiorView===e)})}t("interiorCar").onchange=()=>V(),t("interiorTravel").oninput=e=>a.interior.move(e.target.value),T.querySelectorAll("[data-interior-view]").forEach(e=>{e.onclick=()=>V(e.dataset.interiorView)});function k(e){z=e,a.setCameraMode(f[e]),t("camera").textContent=`VISTA: ${j[f[e]]}`,T.hidden=f[e]!=="interior"}const U=document.createElement("button");U.id="enterInterior",U.className="inspect",U.textContent="INTERIOR DEL TREN",U.onclick=()=>{k(3),V()},t("camera").after(U);const F=document.createElement("button");F.className="secondary",F.textContent="VER INTERIOR",F.onclick=()=>{s.start(),t("intro").style.display="none",k(3),V()},t("inspectIntro").after(F);function w(e){t("toast").textContent=e,t("toast").classList.add("show"),clearTimeout(w.timer),w.timer=setTimeout(()=>t("toast").classList.remove("show"),1700)}function D(e=!1){E=e,d=e}function J(){if(_||a.inspection)return;const e=s.state,n=s.toggleDoors();e.doorsOpen!==n.doorsOpen&&(B=performance.now()),a.doors.update(0,n),I(0,n),e.doorsOpen!==n.doorsOpen?w(n.doorsOpen?"Abriendo puertas del andén":"Cerrando puertas"):e.paused?w("Continúa el servicio para accionar las puertas"):e.speed>.04?w("Detén el tren para abrir las puertas"):e.dwell>0?w("Espera el embarque"):w("Fuera de zona de parada")}function R(){_||(D(!1),b=!1,te=s.state.paused,s.pause(!0),I(),_=!0,t("sourceModal").style.display="grid")}function G(){_&&(_=!1,t("sourceModal").style.display="none",l.querySelectorAll("audio").forEach(e=>{e.pause(),e.currentTime=0}),t("realFrame").src="",t("realFrame").style.display="none",s.pause(te),B=performance.now(),I())}function Y(){a.inspection&&g(),D(!1),b=!1,k(0),P.reset();const e=s.reset();return a.doors.reset(s.state),B=performance.now(),I(),t("intro").style.display="none",t("complete").style.display="none",X=!1,w("Servicio reiniciado"),e}function ne(e){if(a.inspection){e.code==="Escape"&&(e.preventDefault(),g());return}_||e.repeat||!s.state.started||e.target instanceof HTMLInputElement||e.target instanceof HTMLSelectElement||e.target instanceof HTMLTextAreaElement||!["KeyW","ArrowUp","KeyS","ArrowDown","Space","KeyE","KeyC","KeyP","KeyR"].includes(e.code)||(e.preventDefault(),(e.code==="KeyW"||e.code==="ArrowUp")&&(E=!0),(e.code==="KeyS"||e.code==="ArrowDown")&&(d=!0),e.code==="Space"&&(b=!0,w("Freno de emergencia")),e.code==="KeyE"&&J(),e.code==="KeyC"&&k((z+1)%f.length),e.code==="KeyP"&&ae(),e.code==="KeyR"&&Y())}function K(e){(e.code==="KeyW"||e.code==="ArrowUp")&&(E=!1),(e.code==="KeyS"||e.code==="ArrowDown")&&(d=!1)}addEventListener("keydown",ne),addEventListener("keyup",K),addEventListener("blur",()=>{D(!1),b=!1,s.pause(!0),I()}),document.addEventListener("visibilitychange",()=>{document.hidden&&(D(!1),b=!1,s.pause(!0),I(),l.querySelectorAll("audio").forEach(e=>e.pause()),t("realFrame").src="",t("realFrame").style.display="none")}),addEventListener("pagehide",()=>P.update(0,{...s.state,hidden:!0})),t("start").onclick=()=>{s.start(),B=performance.now(),I(),t("intro").style.display="none",w("Caño Amarillo · servicio listo")},t("doors").onclick=J,t("pause").onclick=ae,t("recover").onclick=()=>{s.recover(),w("Tren recuperado · penalización aplicada")},t("reset").onclick=Y,t("camera").onclick=()=>{k((z+1)%f.length)},t("inspect").onclick=()=>{window.location.href="/model-review.html"},t("inspectIntro").onclick=()=>{window.location.href="/model-review.html"},t("inspectStations").onclick=()=>{m()},t("inspectStationsIntro").onclick=()=>{m()},t("sound").setAttribute("aria-pressed","false"),t("sound").onclick=async()=>{const e=++Z,n=!M;if(M=n,t("sound").textContent=n?"CARGANDO AUDIO…":"SONIDO",t("sound").setAttribute("aria-pressed",String(n)),await P.setEnabled(n),e!==Z)return;const r=P.status();ee(r),n&&!r.loading&&!M?w(r.error||(r.total?"No se pudo cargar el audio. Pulsa SONIDO para reintentar.":"Grabaciones originales pendientes · consulta FUENTES")):M&&r.failed.length&&w("Algunas grabaciones no se pudieron cargar. Vuelve a activar SONIDO para reintentar.")},t("volume").oninput=e=>P.setVolume(e.target.value),t("sources").onclick=R,t("share").onclick=async()=>{const e={title:"Metro de Caracas · Línea 1",text:"Conduce el Metro de Caracas desde Caño Amarillo hasta Altamira.",url:window.location.href};try{navigator.share?await navigator.share(e):(await navigator.clipboard.writeText(window.location.href),w("Enlace copiado"))}catch(n){(n==null?void 0:n.name)!=="AbortError"&&w("Copia el enlace del navegador")}},t("completeSources").onclick=R,t("closeSources").onclick=G,t("restart").onclick=Y,document.querySelectorAll("[data-a]").forEach(e=>{const n=()=>{_||a.inspection||(e.dataset.a==="throttle"&&(E=!0),e.dataset.a==="brake"&&(d=!0),e.dataset.a==="emergency"&&(b=!0,w("Freno de emergencia")))},r=()=>{e.dataset.a==="throttle"&&(E=!1),e.dataset.a==="brake"&&(d=!1)};e.onpointerdown=n,e.onpointerup=r,e.onpointercancel=r,e.onpointerleave=r});const fe=((ie=u.at(-1))==null?void 0:ie.distance)||1;let B=performance.now(),re="";function oe(e){var se;if(document.hidden){B=e,requestAnimationFrame(oe);return}const n=Math.max(0,(e-B)/1e3),r=Math.min(n,.05);B=e;const i=s.tick(r,{throttle:E&&a.doorFraction<.001,brake:d,emergency:b});i.speed<=.01&&(b=!1),a.update(i.paused?0:n,i.position,{doorsOpen:i.doorsOpen,doorSide:i.doorSide,speed:i.speed,throttle:E,brake:d,emergency:b}),I(r,i);const L=u[i.target],N=Math.round(i.speed*3.6);t("speed").textContent=String(N).padStart(2,"0"),t("speedbar").style.width=Math.min(100,N/65*100)+"%",t("progress").style.width=Math.min(100,i.position/fe*100)+"%",t("clock").textContent=new Date().toLocaleTimeString("es-VE",{hour12:!1}),t("next").textContent=i.complete?((se=u.at(-1))==null?void 0:se.name)||"—":(L==null?void 0:L.name)||"—",t("distance").textContent=i.missed?"PARADA OMITIDA · RECUPERAR":i.complete?"FIN DE LÍNEA":i.doorsOpen?`EMBARQUE · ${Math.ceil(i.dwell)} s`:`${Math.max(0,Math.round(((L==null?void 0:L.distance)||0)-i.position))} m · ${i.score} PTS`,t("doors").textContent=a.doors.warning?"AVISO DE CIERRE…":a.doors.moving?i.doorsOpen?"ABRIENDO PUERTAS…":"CERRANDO PUERTAS…":i.doorsOpen?"CERRAR PUERTAS · E":"ABRIR PUERTAS · E",t("pause").textContent=i.paused?"CONTINUAR":"PAUSA",t("recover").style.display=i.missed?"":"none";const q=i.paused?"PAUSA":i.doorsOpen?"PUERTAS / EMBARQUE":b?"EMERGENCIA":d?"FRENANDO":E?"TRACCIÓN":"DERIVA",H=L?i.speed*i.speed/(2*2.5):0;t("mode").textContent=`${q} · PARADA ${Math.round(H)} m`,x.querySelectorAll(".station").forEach((le,de)=>{le.classList.toggle("current",de===i.target),le.classList.toggle("passed",de<i.target)}),i.complete&&!X&&(X=!0,t("finalScore").textContent=i.score,t("complete").style.display="grid");const $=`${i.position}:${z}:${i.doorsOpen}:${a.renderRevision}`;$!==re&&(h.render(a.scene,a.camera),re=$),requestAnimationFrame(oe)}["interior","operator"].includes(new URLSearchParams(location.search).get("view"))&&(k(3),V(new URLSearchParams(location.search).get("view")==="operator"?"operator":"saloon")),a.update(0,s.state.position,{doorsOpen:s.state.doorsOpen,doorSide:s.state.doorSide,speed:s.state.speed}),h.render(a.scene,a.camera),requestAnimationFrame(oe),addEventListener("resize",()=>{a.resize(innerWidth,innerHeight),h.setSize(innerWidth,innerHeight),re=""})}Ge().catch(p=>console.error("Metro bootstrap failed",p));
